...
- What is the number of unoccupied homes? (*Hint*: This is not given
  ...
  - 18
- What percent of homes are *occupied*? -((551−18)/551)×100%=96.733%
- Are there any sources of *real* uncertainty in the percent occupied
  you calculated?
  - Yes. The occupancy status of homes could change over time depending
    on when the data was collected as the occupant may leave or join and
    also the time needed to update the status. Since it is based on the
    simulation, it will depend on the code update
...
- Is this sample representative of *all* houses in Helvig? Why or why
  not?
  - Not necessarily. This sample only includes people from the first 25
    homesand is a sequential sample. there maybe a case of the
    neighborhoods being socially clustered like a tradition of relative
    living close, there maybe a over-sample of the cluster that happens
    to be relatives. A better representative sample would be randomly
    selected homes across Helvig and larger sample size.
...
- Which sample—sequential or random—is more *representative* of all
  homes Helvig? Why?
  - random
  - A random sample randomly select the houses with equal chance, which
    helps avoid bias. The sequential sample may over-sample certain
    relatives or families who live near each other. These areas might
    not reflect the overall of the entire island. Random sampling
    reduces this risk and generalize it, thus more representative of all
    homes Helvig
...
- Is there any reason that people with the same last name might tend to
  *live near each other*?
  - there might be a tradition of staying close to family in this
    island, the relative may want to live close for tightness, family
    may want to take care of each other
...
- Note that we used the *random* sample with the bootstrap procedure in
  ...
  - No
  - as sample_seq is not randomly selected and only select the first 25
    home which maybe biased, over-sample some families and not a good
    representative of helvig. Bootstrap assume sample used is
    representative of the general population, thus using sample_seq may
    give misleading confidence interval.
...
- (What population are you going to study?)
  - All of the Islanders? Only the residents of Helvig? Participants
    ...
    *covariates*)? We are going to study the islanders from the topmost
    island in the following cities: Hofn Vardo Helvig Bjurholm Blondous
    Helluland
...
- Net worth -Socioeconomic factors -Occupation -Location (town) -Island
  (cultural ideas\<- to reduce this, we only study one island)
...
- (Write your question / hypothesis here) -How does familial net worth
  influence the number of children in a household?
...
- (What steps will you take to collect the data?)
  ...
  - Be specific. For instance, if you’re going to collect a random
    sample, how will you enumerate all of the things to be sampled? To
    start, we will be taking a random sample of 20 households from each
    city on the topmost island -Each person will take one city (one
    person will take two) -By taking a random sample, we are ensuring
    that the data is representative of the chosen population -Following
    the same protocol as used in challenge eight to generate the sample
  ...
  - Write a protocol. Document step-by-step instructions that your team
    ...
    among the From each parent in each household, we will collect. For
    straight couples parent a will be the woman and parent b will be the
    man, otherwise it doesn’t matter. For one parent households, parent
    a will be the parent and enter NA for parent B: -Name -Occupation
    -Age -Net Worth -Number of Children

  whole team, while making sure each team member doesn’t make ad hoc
  decisions that introduce bias into your results.

- (How will you ensure the data is representative of your chosen
  population?) We will pull data from random form each city, 20 samples
  size

- (For experiments only: How will you ensure any effects you observe are
  due to the treatment, and not due to other factors?) We are pulling
  data on multiple factors not directly related to net worth but
  potentially confounding variables such as occupation, age, and
  location of the household all of which will help search for a mistaken
  correlation/causation.

- (How will you choose your sample size?)

  - This should be a mix of how confident you want to be in your
    results, and what is actually *feasible* for your research team. We
    chose our sample size based on the number of cities and the number
    of households that it’s reasonable for each person to collect data
    on. 20 households times 6 cities creates a total sample size of 120
    households.
